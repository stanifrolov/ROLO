...
class ROLO_utils:
  ...
  def argv_parser(self, argvs):
    ...
    for i in range(1, len(argvs), 2):
      ...
      if argvs[i] == '-input':  self.file_in_path = argvs[i + 1];  self.validate_file_format(self.file_in_path,
                                                                                             allowed_format)
...
def find_yolo_kalman_location(fold, id):
  ...
  yolo_kalman = [float(yolo_kalman_str[0]), float(yolo_kalman_str[1]), float(yolo_kalman_str[2]),
                 float(yolo_kalman_str[3])]
...
def debug_location(img, location):
  ...
  cv2.putText(img_cp, str(location[0]) + ' : %.2f' % location[5], (x - w + 5, y - h - 7), cv2.FONT_HERSHEY_SIMPLEX, 0.5,
              (0, 0, 0), 1)
...
def debug_kalman_locations(img, gt_location, yolo_location):
  ...
  for i in range(2):  # b-g-r channels
    if i == 0:
      ...
      location = yolo_location; color = (255, 0, 0)  # blu3 for yolo_kalman
    x = int(location[0])
    y = int(location[1])
    w = int(location[2])
    h = int(location[3])
